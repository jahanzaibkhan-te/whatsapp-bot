...
const deviceList = (req, res) => {
    fs.readdir('sessions', (err, files) => {
        ...
        const sessionIds = files
            .filter(file => file.startsWith('md_'))
            .map(file => file.replace("creds.json","").replace("md_",""));
    
        response(res, 200, true, "", sessionIds);
    });
}
const status = (req, res) => {
    fs.readFile(
      `sessions/md_${res.locals.sessionId}/creds.json`,
      function (err, data) {
        if (err) {
          const states = [
            "connecting",
...
export { deviceList,find, status, add, del };
